...
from python_coreml_stable_diffusion import controlnet
...
def quantize_weights_to_8bits(args):
    for model_name in [
            "text_encoder", "vae_encoder", "vae_decoder", "controlnet", "unet", "unet_chunk1",
...
def bundle_resources_for_guernika(pipe, args):
    ...
    for source_name, target_name in [("text_encoder", "TextEncoder"),
        ...
        if os.path.exists(source_path):
            ...
            if source_name == "controlnet":
                print("Saving controlnet data")
                controlnet_data = {
                    "identifier": args.controlnet_version,
                ...
                if controlnet_method:
                    controlnet_data["method"] = controlnet_method
                if pipe.text_encoder.config.hidden_size:
                    controlnet_data["hidden_size"] = pipe.text_encoder.config.hidden_size
                with open(os.path.join(target_path, "guernika.json"), 'w', encoding='utf-8') as f:
                    json.dump(controlnet_data, f, ensure_ascii=False, indent=4)
        else:
            logger.warning(
    ...
    if args.unet_in_channels == 4:
        model_data["function"] = "standard"
    elif args.unet_in_channels == 8:
        model_data["function"] = "instructions"
    elif args.unet_in_channels == 9:
        model_data["function"] = "inpaint"
    else:
        model_data["function"] = "unknown"
    if args.text_encoder_hidden_size:
        model_data["hidden_size"] = args.text_encoder_hidden_size
    
    with open(os.path.join(resources_dir, "guernika.json"), 'w', encoding='utf-8') as f:
...
def remove_mlpackages(args):
    for package_name in ["text_encoder", "vae_encoder", "vae_decoder", "controlnet", "unet", "unet_chunk1", "unet_chunk2", "safety_checker"]:
...
def convert_vae_encoder(pipe, args):
    ...
    # Set the output descriptions
    coreml_vae_encoder.output_description["latent"] = "Encoded latent"
...
def convert_controlnet(pipe, args):
    ...
    controlnet.ATTENTION_IMPLEMENTATION_IN_EFFECT = controlnet.AttentionImplementations[
    ...
    cond_shape = (
        batch_size,                    # B
        3,  # C
        (args.output_h or pipe.vae.config.sample_size),  # H
        (args.output_w or pipe.vae.config.sample_size),  # W
    )
...
def convert_unet(pipe, args):
    ...
    # If original Unet does not exist, export it from PyTorch+diffusers
        ...
        print("sample_coreml_inputs", sample_coreml_inputs)
...
def main(args):
    ...
    if args.model_location:
        ...
        pipe = load_pipeline_from_original_stable_diffusion_ckpt(
            ...
            controlnet=args.controlnet_version is not None,
        )
    elif controlnet:
    ...
    args.unet_in_channels = pipe.unet.config.in_channels
    args.text_encoder_hidden_size = pipe.text_encoder.config.hidden_size
